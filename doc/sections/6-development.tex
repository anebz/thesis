...
The first programming task in this thesis has been to implement the BPE algorithm, replicate the results from Sennrich et al.~\cite{sennrich2015neural} and implement the word alignment pipeline, including the mapping from subword alignments to word alignments. In order to modularize the code in clear units, these have been the steps. In the following sections, each step will be explained in detail, with comments and examples for each function.

\begin{enumerate}
    \item Write the learn-BPE from corpus algorithm
    \item Write the apply-BPE to corpus algorithm
    \item Write extract alignments script, including subword alignment to word alignment mapping
...
\subsection{Extract alignments and map subword alignments to word alignments}\label{dev:extractalign}
...
These are the modifications introduced in the \emph{get\_stats} function so that it saves the pair indexes. In the \emph{idx} data structure, not only is the index saved, but also the amount of appearances of each pair in that sentence. This is done to ensure that if the pair ('t', 'h') in index 0 now becomes ('t', 'he') because of the ('h', 'e') merge, and the frequency of ('t', 'h') is reduced by one, it might be assumed that ('t', 'h') no longer appears in that sentence. But this would be a mistake, since there might be other instances of ('t', 'h') in the sentence that are not altered by this merge. It is only relevant to say that ('t', 'h') no longer appears in index 0 when all instances of ('t', 'h') have been merged with other sequences. This is the motivation behind storing the amount of appearances of each pair in a sentence.
